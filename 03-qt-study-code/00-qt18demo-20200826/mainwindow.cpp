    ...
    ui(new Ui::MainWindow)
{
    ...
    // 1. 接收子线程开始信号
    connect(&m_threadUdp,SIGNAL(started()),this,SLOT(onthreadUdp_started()));
    // 2. 接收子线程结束信号
    connect(&m_threadUdp,SIGNAL(finished()),this,SLOT(onthreadUdp_finished()));
    // 3. 定时读取子线程中的数据
    // connect(&mTimer,SIGNAL(timeout()),this,SLOT(onTimeOut()));

    connect(&m_threadWells[0],SIGNAL(started()),this,SLOT(onthreadWells_started()));


    PreProcess();

    // 初始化6个井的线程
    // const_cast<WellsData *>(m_wellsOfSix.at(0).m_Pattern)
    for(int i = 0; i < 6; ++i)
        m_threadWells[i].SetWellsData( &(m_wellsOfSix[i]));
}
...
{//窗口关闭时
    if (m_threadUdp.isRunning())    {
        m_threadUdp.terminate();
        m_threadUdp.wait();
    }
    for(int i = 0; i < 6 && m_threadWells[i].isRunning(); i++){
        m_threadWells[i].ThreadStop();
    }
...
{
    if(m_ArrOfWell[0] >= 0)
        m_wellsOfSix.at(0).m_DetailTableOfWell_2.at(0)->item(0,0)->setText("阵地编号：" + m_ArrOfWell[0]);
...
{
    if(m_ArrOfWell[1] >= 0)
        m_wellsOfSix.at(0).m_DetailTableOfWell_2.at(0)->item(0,0)->setText("阵地编号：" + m_ArrOfWell[1]);
}

void MainWindow::on_pushButton_13_clicked()
{
    if(m_ArrOfWell[2] >= 0)
        m_wellsOfSix.at(0).m_DetailTableOfWell_2.at(0)->item(0,0)->setText("阵地编号：" + m_ArrOfWell[2]);
}

void MainWindow::on_pushButton_21_clicked()
{
    if(m_ArrOfWell[3] >= 0)
        m_wellsOfSix.at(0).m_DetailTableOfWell_2.at(0)->item(0,0)->setText("阵地编号：" + m_ArrOfWell[3]);
}

void MainWindow::on_pushButton_22_clicked()
{
    if(m_ArrOfWell[4] >= 0)
        m_wellsOfSix.at(0).m_DetailTableOfWell_2.at(0)->item(0,0)->setText("阵地编号：" + m_ArrOfWell[4]);
...
{
    if(m_ArrOfWell[5] >= 0)
        m_wellsOfSix.at(0).m_DetailTableOfWell_2.at(0)->item(0,0)->setText("阵地编号：" + m_ArrOfWell[5]);
...
void MainWindow::onthreadUdp_started()
{
    qDebug() << "UDP线程启动" ;
}

void MainWindow::onthreadUdp_finished()
{
    qDebug() << "UDP线程关闭" ;
}

void MainWindow::onthreadWells_started()
{
    qDebug() << "Wells Thread Start." ;
}
...
void MainWindow::Start(){
    ...
    for(int m_indexOfWell = 0;m_indexOfWell < 6;m_indexOfWell++)
        if(m_ArrOfWell[m_indexOfWell] != "-1"){
    ...
    for(int i = 0; i < 6 && m_ArrOfWell[i] != "-1"; ++i){
        // m_threadWells[i].ThreadContinue();
        m_threadWells[i].setThreadStatus(1);
        m_threadWells[i].start();
    }
